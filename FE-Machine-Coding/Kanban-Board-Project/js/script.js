class KanbanBoard {
    ...
    createTicket(task) {
        ...
        this.ticketArr.push({color: this.modalPriorityColor, id, task, status: this.status});
    ...
    renderStoredTickets() {
        for(const ticket of this.ticketArr) {
            ...
            this.appendTicketElement(ticket.id, ticket.color, ticket.task, ticket.status);
        }
    }
    ...
    toggleRemoveFlag() {
        ...
        //TODO: Please select the ticket and that has to remove when your's trash icon is activated via red color.
    }
